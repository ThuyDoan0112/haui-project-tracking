<template>
  ...
  >
    ...
    >
      {{ formatDate(new Date(report.startDate)) }} -
      {{ formatDate(new Date(report.dueDate)) }}</span
    >

    <span
      class="text-primary dark:text-white font-bold text-3xl mt-2 group dark:hover:text-primary-400 transition-[color]"
    >
      {{ report.name }}
    </span>
    <ul
